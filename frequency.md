...
3) 36. Valid Sudoku
	...
	Сколько раз решал: 2
	Время: 24:43
	Комментарий: Создаем три мапы для строк, столбцов квадратов. Ключч в мапе [2]int, где [0] - это номер строки/столюца/
				 квадрата, [1] - значение. Номер квадрата можно посчитать так b := (i / 3) * 3 + (j / 3). Проходимся
				 по всем элементам массива, если '.', то пропускаем, в противном случае создаем ключ и проверяем, есть
				 ли он хоть в одной мапе, если уже есть, то завершаемся с false. Если нет, то добавляем во все мапы.
	Tag: HashMap
...
12) 896. Monotonic Array
	...
	Сколько раз решал: 2
	Время: 07:00
	Комментарий: Создаем две переменные inc, dec, которые будет отслеживать какой тип массива (убывающий или
				 возрастающий). Если оба значение не равны 0, то значит он смешанный, то есть возвращаем false.
	Tag: IterateArray
...
15) 560. Subarray Sum Equals K
	...
	Сколько раз решал: 1
	Время: очень долго
	Комментарий: Мапа хранит префиксные суммы. Текущая сумма элементов минус k - это количство поддмассивов с суммой k,
...
30) Intersection Of Sorted Arrays
	...
	Сколько раз решал: 2
	Время: 06:11
	Комментарий: Сравниваем значения, если равны - записываем в результирующий слайс, уеличиваем оба указателя. Если
				 первое значение меньше, чем второе, то увеличиваем указатель для первого, если нет, то для второго.
	Tag: TwoPointersOnTwoArrays
31) 844. Backspace String Compare
	Линк: https://leetcode.com/problems/backspace-string-compare/
	Сколько раз решал: 2
	Время: 44:39
	Комментарий: Создаем итератора и инициализируем их размерами массивов. Запускаем цикл пока оба итератора больше 0.
				 Вызываем getIndex для каждого итератора, в которую передаем строку и итератор - 1. А результат
				 присваеваем в итератор (таким образом он изменяется). После чего проверяем, что итераторы больше или
				 равны 0 (чтобы не вылезти за пределы массивов) и что значения неодинаковые, если это так, то завершаемся
				 с false. После цикла еще раз вызываем getIndex с выше указанными параметрами и сравниваем возвращенный
				 результат. Он должен быть одинаковый, данное действие проверяет случай, когда букв в строке меньше, чем
				 #. Само функция getIndex принимает строку и итератор и делет следующее: каунтер измнечально равен 0.
				 Дальше идет цикл пока итератор больше или равен 0 и каунтере больше 0 или это #. Внутри цикла, если #,
				 то увеличиваем каунтер и уменьшаем итератор, если зашли по каунтеру, то его уменьшаем и уменьшаем
				 итератор. После цикла возвращаем полученный итератор. Итератор вернет либо указатель на букву или -1.
				 Лучше сначала на бумажке посмотреть как работает getIndex для abc## и a###.
	Tag: TwoPointersOnTwoArrays
32) 283. Move Zeroes
...
45) 33. Search in Rotated Sorted Array
	...
	Сколько раз решал: 2
	Время: 17:13
	Комментарий: Делаем два бинарных поиска, сначала ищем границу сдвига. А потом ищем сам таргет. Второй поиск идет
				 по "сдвинутому" массиву, то есть правая граница это len(nums) + right. Но тогда стоит помнить, что
				 можно выйти за границы, поэтому проверяем как nums[mid%len(nums)]. И не забыть, что полученный ответ
				 тоже может быть загарницей, поэтому nums[left%len(nums)].
	Tag: BinarySearch
46) 69. Sqrt(x)
...
69) 1249. Minimum Remove to Make Valid Parentheses
	...
	Сколько раз решал: 2
	Время: 45:31
	Комментарий: В стек добовляются индексы открывающихся скобок. Если встречается закрыающаяся скобка, то проверяем
				 размер стека, если он больше 0, то удаляем последний элемент из стека (так как нашлась пара). Если же
				 стек оказался пустым, то помечаем # текущий элемент строки (луше сразу переделать в слайс байт, чтобы
				 мочь изменять). После того, как прошли всю строку запускаем цикл по стеку, так как там могут остаться
				 "одинокие" открывающиеся строки - помечаем их # в строке. Удаляем из строки # - это наш ответ.
	Tag: Stack
...
74) 986. Interval List Intersections
	...
	Сколько раз решал: 2
	Время: 22:26
	Комментарий: Создаем два итератора на первый и второй списки. Запускаем цикл пока оба итератора меньше, чем длины
				 списков. Находи левую и правую границы пересечения (выбираем большую из левых, и меньшую из правых).
				 Проверяем, что левая меньше или равна правой, если так, то добавляем в ответ. Находим где правая
				 граница меньше и двигаем итератор для этого списка.
	Tag: Intervals
...
78) 394. Decode String
	...
	Сколько раз решал: 2
	Время: 01:04:18
	Комментарий: Сложно. Проходимся по строке и складываем в стек, пока не ]. Как только встречаем, то начинаем
...
94) 130. Surrounded Regions
	...
	Комментарий: Решал рекурсивным DFS. Сначала запустил DFS для всех краевых узлов, пометив острова у краев посещенными.
				 После запустил DFS, чтобы заменить 'O' на 'X', если это 'O' и если оно не было посещено.
	Tag: DFS
...
96) 1091. Shortest Path in Binary Matrix
	...
	Tag: BFS
97) Longest Substring with At Most K Distinct Characters
	Линк: https://www.lintcode.com/problem/386/
	Сколько раз решал: 2
	Время: 34:42
	Комментарий: Основной цикл основать на том, что правый указатель меньше длины строки. Увеличиваем в мапе
				 каунтер символа, на котором стоит правый указатель. А теперь проверяме, что размер мапы больше k, если
				 это так, то начинаем двигать левый указатель в цикле, до тех пор пока мапа не станет нужного размера.
				 Важно, что когда двигаем указатель, сначала уменьшаем значение записанное в мапе и только если оно равно
				 0, то удаляем его из мапы. Когда выходим из внутреннего цикла считаем длину и сравниваем с максимальной.
				 Важно, что к длине нужно добавить 1, так как левый указатель мы уже подвинули.
	Tag: Non-Overlaping SlidingWindows
98) Найти в массиве наименьшее произведение двух чисел
	Линк: https://github.com/MomsEngineer/error_diary/blob/main/task_98_test.go
	Сколько раз решал: 2
	Время: 14:35
	Комментарий: Находим два минимальных и максимальных значения и возвращаем перемножение этих занчений в зависимости
				 от того, все ли ортрицательные (min1 < 0 && max1 < 0) или если отрицательные (min1 < 0 && max1 >= 0)
				 или все положительные (min1 >= 0).
	Tag: FindMax, FindMin
99) Вернуть массив с элементами первого массива, которых нет во втором
	Линк: https://github.com/MomsEngineer/error_diary/blob/main/task_99_test.go
	Сколько раз решал: 1
	Время: 23:06
	Комментарий: Делаем два указателя на кажый массив. Идем пока указатели не выходят из своих строк. Если значение в
				 первом указателе меньше, то добавляем в выходной массив и двигаем указатель. Если занчение больше,
				 то двигаем второй указатель. Если они равны, то двигаем ТОЛЬКО первый указатель (это важно, потому
				 что могут быть повторяющиеся элементы и нам нужно их пропустить).
	Tag: TwoPointers